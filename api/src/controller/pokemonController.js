...
module.exports = {
  indexAndName: async (req, res) => {
    if (req.query.name) {
      axios
        .get(`${URL_GET}${req.query.name.toLowerCase()}`)
        .then((data) => data.data)
        .then((data) =>
          res.send({
            id: data.id,
      ...
      try {
        ...
        const infoFirstRequest = await axios.all(
          firstRequest.data.results.map((elem) => axios.get(elem.url))
        );
        const infoSecondRequest = await axios.all(
          secondRequest.data.results.map((elem) => axios.get(elem.url))
        );
        const fullRequestApi = [...infoFirstRequest, ...infoSecondRequest];
        let fullInfoApi = fullRequestApi.map((elem) => ({
          ...
          types: elem.data.types.map((elem) => {
            return { name: elem.type.name };
          }),
        }));
  ...
  create: async (req, res) => {
    let { name, life, atack, defense, speed, weight, height, type, img, id } =
      req.body;
    try {
